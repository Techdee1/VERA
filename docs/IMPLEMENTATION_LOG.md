...
- `docker compose exec postgres psql -P pager=off -U vera -d vera -c "\\dt"` showed all 6 expected tables.
- `docker compose exec postgres psql -P pager=off -U vera -d vera -c "SELECT unnest(enum_range(NULL::decision_status));"` returned all 4 expected enum values.
