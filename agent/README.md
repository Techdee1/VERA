...
VERA is an AML-focused Lua AI agent that analyzes transaction flows, detects suspicious patterns, scores explainable risk, and produces Suspicious Transaction Report drafts in `PENDING_REVIEW` status for human compliance review. It is upgraded from the original GRACE agent.
